...
const string GIS::logFilePath = "errors.log";

const double GIS::precision = 0.1;
...
std::size_t GIS::saveMapFile(const std::string& filename) const{
    ...
    try {
        ...
        for (const auto &entityIt:entityById) {//get all the GIS Entities
            entityVector.push_back(entityIt.second);
        }
        NumOfSavedEntities= entityJsonHandler.saveEntities(filename, entityVector);
    }
    catch (const std::exception& e) {
...
std::vector<EntityId> GIS::getEntities(const std::string& search_name) const {
    ...
    try {
        ...
        for (auto &it:vector) {
            currEntity = it;
            entitiesId.push_back(currEntity->getId());
        }
    }
    catch (const std::exception& e) {
...
std::vector<EntityId> GIS::getEntities(const std::string& search_name, const Coordinates& cord, Meters radius) const{
    vector<EntityId> ids;
...
std::optional<Coordinates> GIS::getEntityClosestPoint(const EntityId& entityId, const Coordinates& coordinates) const{
    try {
...
std::pair<Coordinates, EntityId> GIS::getWayClosestPoint(const Coordinates& coordinates) const{
    return getWayClosestPoint(coordinates, Restrictions(""));
}
...
std::pair<Coordinates, EntityId> GIS::getWayClosestPoint(const Coordinates& coordinates, const Restrictions& res) const{

    std::pair<Coordinates, EntityId> closestWay =
            std::pair<Coordinates, EntityId>(Coordinates(Longitude(404), Latitude(404)), EntityId(""));
    if(waysLoaded == 0){
        return closestWay;
    }
    try {
        unordered_set<GridEntity> foundGeometries;
        SpiralCoordinatesTraverser traverser = SpiralCoordinatesTraverser(coordinates, precision);
        Meters minCellDistance = Meters(0);
        ...
        while ( minCellDistance <=  minWayDistance*buffer || !wayFound) {
            ...
            do {
                auto cell = traverser.getNext();
                Meters distance = GridMath::getMinDistance(cell, coordinates, precision);
                if (!minCellDistanceInit ||  distance < minCellDistance) {
                    minCellDistanceInit = true;
                    minCellDistance = distance;
                }
                if ( distance <= minWayDistance || !wayFound) {
                    ...
                    for (const auto &way : ways) {
                        if (!foundGeometries.contains(way)) {
                            ...
                            Meters distanceFromEntity = way.getDistanceFrom(coordinates);
                            if (( distanceFromEntity <= minWayDistance || !wayFound)
                            &&(!res.isWayRestricted(*dynamic_pointer_cast<Way>(way.getEntity())))) {
                                wayFound = true;
                                minWayDistance = distanceFromEntity;
                                Coordinates closestPoint = way.getClosestPont(coordinates);
                                closestWay.first = closestPoint;
                                closestWay.second = way.getEntityId();
                            }
                        }
                    }
                }
...
const Way& GIS::getWay(const EntityId& wayId) const{
    ...
    auto returnWay = dynamic_pointer_cast<Way>(entityById.at(wayId));
    return *returnWay;
}
