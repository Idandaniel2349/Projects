...
public:
    ...
    const Coordinates& getFromJunctionCoordinates() const{return fromCords;};
    const Coordinates& getToJunctionCoordinates() const{return toCords;};
    ...
    bool isToll() const {return toll_road;};

    bool isBidirectional() const {return wayDirection==bidirectional;};

    const vector<restricted>& getRestrictions() const {return restrictions;};

    Meters getLength() const;

    bool isHighway() const{return highway;};

    std::pair<EntityId, EntityId> getJunctions() const{return std::pair<EntityId, EntityId>(from,to);};
